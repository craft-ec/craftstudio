...
impl DaemonManager {
    ...
    pub fn start(&self, config: DaemonConfig) -> Result<DaemonInstance, String> {
        ...
        // Only write a default config if one doesn't exist yet (first run).
        // The daemon config file is the source of truth — CraftStudio should not
        // overwrite it on every start. The daemon reads its own config.json.
        {
